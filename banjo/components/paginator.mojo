...
struct KeyMap(Copyable):
    ...
    def __init__(out self):
        ...
        self.prev_page = Binding(
            [press(PageUp()), press(Left()), press(Char("h"))],
            Help("h/left", "previous page"),
        )
        self.next_page = Binding(
            [press(PageDown()), press(Right()), press(Char("l"))],
            Help("l/right", "next page"),
        )
...
struct Paginator(Copyable):
    ...
    def __init__(
        ...
        self.active_dot = String("•")
        self.inactive_dot = String("○")
        self.separator = String("/")
        self.keymap = KeyMap()

    def set_total_pages(mut self, items: Int) -> Int:
    ...
    def slice_bounds(self, length: Int) -> Tuple[Int, Int]:
        ...
        """
        var start = self.page * self.per_page
