...
from banjo.multiplex.select import EVENT_READ, EVENT_WRITE, FileDescriptorBitSet, timeval
...
struct KQueueSelector:
    ...
    fn register(mut self, fd: Int, events: Int) raises -> None:
        ...
        if events & EVENT_READ:
            ...
            var kev = kevent(self._selector, UnsafePointer(to=changelist), 1, eventlist, 0, UnsafePointer(to=timeout))
            if kev == -1:
